<?php
// boundaries/SuspendUserAccountConfirmationPopUp.php
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['confirm']) && $_POST['confirm'] === 'yes') {
        $result = $controller->suspendUserAccount($aID);
    } else {
        // User cancelled — redirect back to the admin menu
    ...
    if ($result) {
        $_SESSION['flash_msg'] = "Account suspended successfully!";
    } else {
        $_SESSION['flash_msg'] = "Failed to suspend account.";
    }
    header("Location: ../index.php?action=UserAccountMenuPage");
    exit;
}
...
<body>
    <div class="overlay">
        <div class="popup-container">
            <h2>Are you sure you want to suspend this account?</h2>
            <form method="post">
